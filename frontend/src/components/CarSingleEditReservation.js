...
import { getCarSingleReservation, 
...
function CarSingleEditReservation() {
    ...
    const carUpdateReservation = useSelector(state => state.carUpdateReservation)
    const { success, error:errorUpdate, reservation:reservationUpdate  } = carUpdateReservation
    ...
    const submitHandler = (data) => {
        ...
        } else if(startDateTimeCombiner.valueOf() < (new Date().valueOf() + SET_DATE_TIME_RESERVATION)) {
    ...
    useEffect(() =>{
        scroller.scrollTo('navbar', {smooth: true, offset: -90,duration: 10,})
        if(!error){
            if(!reservation.client_name || reservation.id !== Number(ReservationId)){
                ...
                setNewEvent({ ...newEvent, 
                    start: new Date(`${reservation.start_year}-${reservation.start_month}-${reservation.start_day}`),
                ...
                if(!getStartTimeHoursAndMinutes){
                    setStartTimeValue(new Date(`01/02/2021 ${reservation.start_hour}:${reservation.start_minute}`))
            ...
            {errorMessage ? <Message variant='danger'>{errorMessage}</Message> : null}
                <Row>
                    <Col>
                        {reservation.id
                            ...
                            <div>
                                ...
                                <Image src = {reservation.id_cars.image} className='car-show-img-sizing'/>
                                <h4>{reservation.id_cars.short_name}</h4>
                                <h5>{CAR_SINGLE_EDIT_RESERVATION_REGISTRATION_NO} {reservation.id_cars.code_registration}</h5>     
                                <hr /> 
                            </div>
                            : null
                        }
                    ...
                    <Col className='btn-position'>
                        ...
                        <LinkContainer to={`/car/${carId}/show/${action}/${locationId}/id-res/${ReservationId}`}>  
                            <Button 
                                variant='info' 
                                className='m-1'
                            >
                ...
                <Form onSubmit={handleSubmit(submitHandler)}>
                    <Form.Group controlId='name'>
                        <Form.Label>{CAR_SINGLE_EDIT_RESERVATION_CUSTOMER_NAME_TITLE}</Form.Label>
                        <Form.Control
                            type='text'
                    ...
                    <Form.Group controlId='idDoc' >
                        <Row >                                  
                           <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_DUCUMENT_TYPE_TITLE}</Form.Label>
                                <Form.Select 
                                    ...
                                    className='form-reservation'
                                >
                                    <option value={CAR_SINGLE_EDIT_RESERVATION_OPTION_0} hidden></option>
                                    <option value={CAR_SINGLE_EDIT_RESERVATION_OPTION_PASSPORT}>{CAR_SINGLE_EDIT_RESERVATION_OPTION_PASSPORT}</option>
                                    <option value={CAR_SINGLE_EDIT_RESERVATION_OPTION_ID}>{CAR_SINGLE_EDIT_RESERVATION_OPTION_ID}</option>
                                    <option value={CAR_SINGLE_EDIT_RESERVATION_OPTION_DRIVING_LICENSE}>{CAR_SINGLE_EDIT_RESERVATION_OPTION_DRIVING_LICENSE}</option>
                                    <option value={CAR_SINGLE_EDIT_RESERVATION_OPTION_OTHER}>{CAR_SINGLE_EDIT_RESERVATION_OPTION_OTHER}</option>
                                </Form.Select>
                            ...
                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_DUCUMENT_NO_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("IdNumber")}                               
                    ...
                    <Form.Group controlId='idPhoneEmail' >
                        <Row >                                  
                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_PHONE_NO_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("phone",
                            ...
                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_EMAIL_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("email", 
                    ...
                    <Form.Group controlId='datePicker' >
                        <Row >
                            <Col md={4} xs={8}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_RENT_FROM_TITLE}</Form.Label>
                                <DatePicker
                            ...
                            <Col md={2} xs={4}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_TIME_TITLE}</Form.Label>
                                <TimePickerComponent
                                    id="timepicker" 
                                    placeholder="" 
                                    format = "HH:mm"
                            ...
                            <Col md={4} xs={8}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_RENT_TO_TITLE}</Form.Label>
                                <DatePicker 
                                    dateFormat="yyyy-MM-dd" 
                                    autoComplete='off'
                                    autocomplete="off"
                    ...
                    <Form.Group controlId='localisation'>
                        <Row>
                            <h5>{CAR_SINGLE_EDIT_RESERVATION_LOCATISATION_SUBTITLE} {locationInfo.short_name}</h5>
                            <Col md={8} xs={8}>
                                <Form.Label className="mt-3">{CAR_SINGLE_EDIT_RESERVATION_SUBTITLE}</Form.Label>
                                <Form.Select
                                    aria-label="Default select example"
                                    name = 'location'
                                    aria-label="Default select example"
                    ...
                    >
                        <FontAwesomeIcon icon={faAddressBook} /> {CAR_SINGLE_EDIT_RESERVATION_BUTTON_NAME} 
                    </Button>
                </Form>
            </FormContainer>
        </main>
