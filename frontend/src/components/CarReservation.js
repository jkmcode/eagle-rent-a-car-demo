...
import { 
    ...
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_2,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_3,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_4,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_5,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_6,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1_MSG,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_2_MSG,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_3_MSG,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_4_MSG,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_5_MSG,
    CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_6_MSG,
...
function CarReservation() {
    ...
    const carReservationCreate = useSelector(state => state.carReservationCreate)
    const { loading, car, error } = carReservationCreate

    const carDetails = useSelector(state => state.carDetails)
    const { loading:loadingCarDetails, car:carCarDetails, error:errorCarDetails} = carDetails
    ...
    useEffect(() => {
        ...
        if(car===CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1){
            setDateMsg(CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1_MSG)
        } 
        if(car===CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_2){
            setDateMsg(CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_2_MSG)
        } 
        if(car===CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_3){
            setDateMsg(CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_3_MSG)
        } 

        if(car===CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_4){
            setDateMsg(CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_4_MSG)
        } 
        if(car===CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_5){
            setDateMsg(CAR_RESERVATION_ERROR_HANDLING_EXIST_RANGE_DATE_EX_5_MSG)
        } 
                ...
                <Row>
                    <Col>
                        <div>
                            ...
                            <Image src = {carCarDetails.image} className='car-show-img-sizing'/>
                            <h4>{carCarDetails.short_name}</h4>
                            <h5>{CAR_RESERVATION_REGISTRATION_NO} {carCarDetails.code_registration}</h5>     
                            <hr /> 
                        </div>
                    </Col>
                ...
                <Form onSubmit={handleSubmit(submitHandler)}>
                    <Form.Group controlId='name'>
                        ...
                        <Form.Control
                            ...
                            {clientNameMsg ? <p className='docTypeMessage-style'>{clientNameMsg}</p> : null}
                    ...
                    <Form.Group controlId='idDoc' >
                        <Row >                                  
                           <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_DUCUMENT_TYPE_TITLE}</Form.Label>
                                <Form.Select 
                                ...
                                >
                                    ...
                                    <option value={CAR_RESERVATION_OPTION_PASSPORT}>{CAR_RESERVATION_OPTION_PASSPORT}</option>
                                    <option value={CAR_RESERVATION_OPTION_ID}>{CAR_RESERVATION_OPTION_ID}</option>
                                    <option value={CAR_RESERVATION_OPTION_DRIVING_LICENSE}>{CAR_RESERVATION_OPTION_DRIVING_LICENSE}</option>
                                    <option value={CAR_RESERVATION_OPTION_OTHER}>{CAR_RESERVATION_OPTION_OTHER}</option>
                                </Form.Select>

                                {docTypeMessage ? <p className='docTypeMessage-style'>{docTypeMessage}</p> : null}
                            </Col>

                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_DUCUMENT_NO_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("IdNumber")}                               
                                ...
                                {nrDocMsg ? <p className='docTypeMessage-style'>{nrDocMsg}</p> : null}
                            </Col>
                        </Row>
                    </Form.Group> 
                    ...
                    <Form.Group controlId='idPhoneEmail' >
                        <Row >                                  
                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_PHONE_NO_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("phone",
                            ...
                            <Col md={6} xs={12}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_EMAIL_TITLE}</Form.Label>
                                    <Form.Control
                                        type="text"
                                        className='form-reservation'
                                        {...register("email", 
                    ...
                    <Form.Group controlId='datePicker' >
                        <Row >
                            <Col md={4} xs={8}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_RENT_FROM_TITLE}</Form.Label>
                                <DatePicker
                            ...
                            <Col md={4} xs={8}>
                                <Form.Label className="mt-3">{CAR_RESERVATION_RENT_TO_TITLE}</Form.Label>
                                <DatePicker 
                                    dateFormat="yyyy-MM-dd" 
                                    autoComplete='off'
                    ...
                    <Form.Group controlId='note'>
                        <Row>
                            <Col md={9} xs={12}>
                              <Form.Label className="mt-3">{CAR_RESERVATION_NOTE_TITLE}</Form.Label>
                              <Form.Control
                                as="textarea"
                                name="note"
                                {...register("note")}
