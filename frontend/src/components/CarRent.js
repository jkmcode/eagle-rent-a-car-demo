...
function CarRent() {
  ...
  const submitHandler = (data) => {
    ...
    if (!endDateTimeCombiner) {
      ...
      endDateTimeCombiner.valueOf() - MIN_DURATION <
      new Date().valueOf()
    ) {
  ...
  useEffect(() => {
    ...
    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_1) {
      setDateMsg(ERROR_HANDLING_EXIST_RANGE_DATE_EX_1_MSG);
    }
    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_2) {
      setDateMsg(ERROR_HANDLING_EXIST_RANGE_DATE_EX_2_MSG);
    }
    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_3) {
      setDateMsg(ERROR_HANDLING_EXIST_RANGE_DATE_EX_3_MSG);
    }

    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_4) {
      setDateMsg(ERROR_HANDLING_EXIST_RANGE_DATE_EX_4_MSG);
    }
    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_5) {
      setDateMsg(ERROR_HANDLING_EXIST_RANGE_DATE_EX_5_MSG);
    }
    if (rent === ERROR_HANDLING_EXIST_RANGE_DATE_EX_6) {
  ...
  return (
    <main>
      ...
      <FormContainer>
        ...
        {actionResToRent ? (
          <h3>{CAR_RENT_RENT_BASED_ON_RESERVATION}</h3>
        ) : (
          <h3>{CAR_RENT_TITLE}</h3>
        )}
        ...
        <Row>
          <Col>
            ...
            <h4>{car.name}</h4>
            <h5>
              {CAR_RENT_NO_REGISTRATION_SUBTITLE} {car.code_registration}
            </h5>
          </Col>

          <Col className="btn-position">
            <LinkContainer to={`/mainpage/${locationId}/car-list/`}>
              <Button variant="warning" className="btn-back m-1">
                <FontAwesomeIcon icon={faAngleDoubleLeft} /> {BTN_BACK}
              </Button>
            </LinkContainer>
            {actionResToRent ? (
              <LinkContainer
                to={`/car/${car.id}/show/${actionResToRent}/${resId}/${locationId}`}
              >
        ...
        <Form onSubmit={handleSubmit(submitHandler)}>
          <Form.Group controlId="name">
            ...
            ></Form.Control>
            {clientNameMsg ? (
              <p className="message-style">{clientNameMsg}</p>
            ) : null}
            {errors.name && (
          ...
          <Form.Group controlId="idDoc">
            <Row>
              <Col md={6} xs={12}>
                ...
                {docTypeMessage ? (
                  <p className="form-msg-style">{docTypeMessage}</p>
                ) : null}
              </Col>
            ...
            <Row>
              ...
              <Col md={2} xs={4}>
                ...
                <TimePickerComponent
                  ...
                  format="HH:mm"
                  value={
                    actionResToRent ? endTimeValue : TIME_DEFAULT_VALUE_END
                  }
                  min={TIME_MIN_VALUE}
                  max={TIME_MAX_VALUE}
                  step={TIME_STEP}
          ...
          <Form.Group controlId="deposit">
            ...
            <Row>
              <Col md={4} xs={4}>
                ...
                ></Form.Control>
                {depositMsg ? (
                  <p className="form-msg-style">{depositMsg}</p>
                ) : null}
                {errors.deposit && (
                  <div className="form-msg-style">{errors.deposit.message}</div>
                )}
              </Col>
              <Col md={2} xs={4}>
                <Form.Label className="mt-3">
                ...
                {depositCurrency ? (
                  <p className="form-msg-style">{depositCurrency}</p>
                ) : null}
              </Col>
              <Col md={3} xs={2}>
          ...
          <Form.Group controlId="totalPrice">
            <Row>
              <Col md={4} xs={4}>
                ...
                ></Form.Control>
                {totalPriceMsg ? (
                  <p className="form-msg-style">{totalPriceMsg}</p>
                ) : null}
                {errors.totalPrice && (
              ...
              <Col md={2} xs={4}>
                ...
                {totalPriceCurrency ? (
                  <p className="form-msg-style">{totalPriceCurrency}</p>
                ) : null}
              </Col>
              <Col md={3} xs={2}>
                <Form.Label className="mt-3"></Form.Label>
          ...
          <Form.Group controlId="takeBack">
            ...
            <Row>
              <Col md={8} xs={8}>
                <h5>
                  {CAR_RENT_LOCATISATION_SUBTITLE} {locationInfo.short_name}
                </h5>
                <Form.Label className="mt-3">
                  {CAR_RENT_LOCATION_TITLE}
                </Form.Label>
          ...
          <Button type="submit" variant="info" className="rounded my-3">
