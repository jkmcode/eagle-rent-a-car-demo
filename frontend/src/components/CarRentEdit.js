...
function CarRentEdit() {
  ...
  const {
    loading: loadingCarRentDetails,
    rent,
    error: errorCarRentDetails,
  } = carRentDetails;

  const carEditRent = useSelector((state) => state.carEditRent);
  const {
    loading: loadingCarEditRent,
    rent: rentCarEditRent,
    error: errorCarEditRent,
  } = carEditRent;
  ...
  useEffect(() => {
    ...
    if (
      rentCarEditRent === CAR_RENT_EDIT_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1
    ) {
      setDateMsg(CAR_RENT_EDIT_ERROR_HANDLING_EXIST_RANGE_DATE_EX_1_MSG);
    }

    if (
      rentCarEditRent === CAR_RENT_EDIT_ERROR_HANDLING_EXIST_RANGE_DATE_EX_2
    ) {
          ...
          <section>
            <Row>
              ...
              <Col className="btn-position">
                ...
                <LinkContainer
                  to={`/car/${car.id}/show/${action}/${locationId}/id-rent/${rentId}`}
                >
            ...
            <Row>
              <Col>
                <h4 className="general-info-subtitle">
                  {CAR_RENT_EDIT_GENERAL_INFO_SUBTITLE}
                </h4>
                {!lessGeneralInfo ? (
                  <Button className="btn-show" onClick={showGeneralInfo}>
            ...
            {generalInfo ? (
              <div>
                <h5>
                  {CAR_RENT_EDIT_CLIENT_NAME} {rent.client_name}
                </h5>
                <h5>
                ...
                <h5>
                  {CAR_RENT_EDIT_PHONE} {rent.client_phone}
                </h5>
                {rent.client_email ? (
                  <h5>
                    {CAR_RENT_EDIT_EMAIL} {rent.client_email}
                  </h5>
                ) : null}
                <h5>
                  {CAR_RENT_EDIT_DATE_FROM}
            ...
            <Row>
              <Col>
                <h4 className="payment-info-subtitle">
                  {CAR_RENT_EDIT_PAYMENTS_INFO_TITLE}
                </h4>
                {!lessPaymentlInfo ? (
                  <Button className="btn-show" onClick={showPaymentInfo}>
                    <FontAwesomeIcon icon={faArrowCircleDown} /> {BTN_SHOW_MORE}
                  </Button>
                ) : (
                  <Button className="btn-show" onClick={showPaymentInfo}>
            ...
            {paymentInfo ? (
              <div>
                <h5>
                  {CAR_RENT_EDIT_DEPOSIT} {rent.deposit} {rent.deposit_currency}
                </h5>
                {!rent.total_price_is_paid ? (
                  <h5>
                    {CAR_RENT_EDIT_TOTAL_PRICE} {rent.total_price}{" "}
              ...
              <Form.Group controlId="totalPrice">
                <Row>
                  <Col md={4} xs={6}>
                    ...
                    {totalPriceMsg ? (
                      <p className="docTypeMessage-style">{totalPriceMsg}</p>
                    ) : null}
                  </Col>
                  <Col md={2} xs={4}>
                    <Form.Label className="mt-3">
              ...
              <Form.Group controlId="localisation">
                <Row>
                  ...
                  <Col md={8} xs={8}>
                    ...
                    {location ? (
                      <p className="form-msg-style">{location}</p>
                    ) : null}
                  </Col>
                </Row>
              </Form.Group>
